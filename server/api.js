...
module.exports = {
  ...
  getArticleAll (req, res, _next) {
    pool.getConnection((_err, connection) => {
      ...
      let noDel = params.noDel
      let limit = params.limit
      let start = limit > LIMIT ? limit - LIMIT : 0
      let sqlGetLen = '; SELECT COUNT(1) AS total FROM articles WHERE state = 1'
      let sql = sqlMap.article.queryAll + (limit ? ' LIMIT ' + start + ',' + limit : '')
  ...
  getArticleById (req, res, _next) {
    ...
    pool.getConnection((_err, connection) => {
      connection.query(sqlMap.article.queryById, [id], (_err, result) => {
        console.log('getArticleById : ' + sqlMap)
        if (result.length !== 0) {
  ...
  getArticleByTagId (req, res, _next) {
    pool.getConnection((_err, connection) => {
      let params = req.query
      ...
      let limit = params.limit
      let start = limit > LIMIT ? limit - LIMIT : 0
      let sqlGetLen = '; SELECT COUNT(1) AS total FROM tag_links WHERE tid = ' + tagId
      let sql = sqlMap.article.queryByTagId + (limit ? ' LIMIT ' + start + ',' + limit : '')
      sql += (limit && limit <= LIMIT) ? sqlGetLen : ''
      connection.query(sql, [tagId], (_err, result) => {
        let total = 0
        let data
        if (limit && limit <= LIMIT) {
          data = result[0]
  ...
  updArticle (req, res, _next) {
    pool.getConnection((_err, connection) => {
      ...
      let tagIdMax = postData.tagIdMax
      let sql = 'UPDATE articles SET '
      let str = ''
      connection.query('SELECT * FROM articles WHERE title = ? AND id != ?', [title, id], (_err, result) => {
        if (result.length === 0) {
          if (postData.state === 3) { // 彻底删除
            sql = 'DELETE FROM articles WHERE id = ' + id + '; '
            sql += 'DELETE FROM tag_links WHERE aid = ' + id + '; '
          } else {
            let markedHtml, htmlTags
  ...
  // search
  getSearchArticle (req, res, next) {
    
  },
  // 阅读量、点赞量 +1
  addViewOrStart (req, res, _next) {
    pool.getConnection((_err, connection) => {
      let postData = req.body
      let id = postData.id
      let count = postData.count
      let type = postData.type
      let sql = 'UPDATE articles SET ' + type + ' = ' + count + ' WHERE id = ' + id
      connection.query(sql, (_err, _result) => {
        res.send('ok')
      })
    })
  },
  // 新增文章
  addArticle (req, res, _next) {
    pool.getConnection((_err, connection) => {
      let postData = req.body
      ...
      connection.query(sqlMap.article.queryByTitle, [title], (_err, result) => {
        if (result.length === 0) {
          if (req.session.userData.root !== 1) {
            postData.state = 2
          }
          connection.query(sqlMap.article.insert + '; SELECT MAX(id) as id FROM articles', [postData.user_id, title, postData.state, postData.type, postData.loadURL, summary, curTime, null, 0, 0], (_err, result) => {
  ...
  // 标签
  getTagAll (_req, res, _next) {
    pool.getConnection((_err, connection) => {
      connection.query(sqlMap.tag.queryAll, (_err, result) => {
        res.json(result)
        connection.release()
      })
    })
  },
  getTagById (_req, res, _next) {
    pool.getConnection((_err, connection) => {
      connection.query(sqlMap.tag.queryById, (_err, result) => {
        res.json(result)
        connection.release()
      })
    })
  ...
  writeComment (req, res, _next) {
    pool.getConnection((_err, connection) => {
      ...
      let website = postData.website
      let reminder = postData.reminder
      let avatar = postData.avatar || ''
      let uid = postData.uid
      if (uid) {
        connection.query(sqlMap.comment.insert, [postData.aid, uid, postData.rid, postData.rCommentID, postData.content, time], (_err, _result) => {
          postData.uid = uid
          res.json(postData)
          connection.release()
        })
      } else {
        connection.query(sqlMap.visitor.insert, [name, email, website, avatar, reminder], (err, _result) => {
          if (err) { throw err }
          connection.query(sqlMap.visitor.queryByEmail, [email], (err, result) => {
            if (err) { throw err }
            uid = result[0].id
            connection.query(sqlMap.comment.insert, [postData.aid, uid, postData.rid, postData.rCommentID, postData.content, time], (_err, result) => {
              postData.uid = uid
  ...
  getComment (req, res, _next) {
    pool.getConnection((_err, connection) => {
      let params = req.query
      let obj = {}
      let arr = []
      connection.query(sqlMap.comment.queryByActicleId, [params.articleID], (_err, result) => {
        for (let i = 0, len = result.length; i < len; i++) {
          let tmp = result[i]
          let replyCommentID = tmp.reply_comment_id
          let replyID = tmp.reply_id
          let tmpObj = {}
          if (!replyCommentID) {
  ...
  // 上传图片
  uploadPic (req, res, _next) {
    let form = new formidable.IncomingForm()
    let userID, userAvatar
    ...
    form.parse(req, (_error, fields, files) => {
      ...
      for (let key in files) {
        ...
        pool.getConnection((_err, connection) => {
          connection.query(sqlMap.user.update, [fileName, userID], (err, _result) => {
            if (err) {
              throw err
            }
  ...
  // 用户
  getUserAll (_req, res, _next) {
    pool.getConnection((_err, connection) => {
      connection.query(sqlMap.user.queryAll, (_err, result) => {
        res.json(result)
        connection.release()
      })
    })
  ...
  // 游客评论邮箱查询
  getVisitor (req, res, _next) {
    pool.getConnection((_err, connection) => {
      let params = req.query
      let email = params.email
      connection.query(sqlMap.visitor.queryByEmail, [email], (_err, result) => {
    ...
    pool.getConnection((_err, connection) => {
      ...
      connection.query(sqlMap.user.queryByEmail, [email], (err, result) => {
        ...
        if (result.length !== 0) {
          ...
          connection.query(sqlMap.visitor.updByEmail, [postData.name, postData.website, avatar, postData.reminder, email], (err, _result) => {
            if (err) { throw err }
            res.json(true)
            connection.release()
          })
  ...
  // 登陆
  userLogin (req, res, _next) {
    pool.getConnection((_err, connection) => {
      let postData = req.body
      connection.query(sqlMap.user.queryByEmail, [postData.email], (_err, result) => {
        if (result.length === 0) {
          res.json({
            status: false,
            msg: '用户不存在'
    ...
    pool.getConnection((_err, connection) => {
      ...
      connection.query(sqlMap.user.queryByEmail, [postData.email], (_err, result) => {
        if (result.length !== 0) {
          ...
          let website = postData.website
          let reminder = postData.reminder
          connection.query(sqlMap.user.insert, [name, email, password, avatar, website, reminder, name, email, avatar, website, reminder], (err, _result) => {
            let status = true
            let msg = '注册成功'
            if (err) {
              status = false
